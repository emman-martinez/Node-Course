...
function getStagedFiles() {
  ...
  return output
    ...
    .filter(Boolean)
    .map((file) =>
      file.startsWith(projectFolder) ? file.slice(projectFolder.length) : file,
    );
}
...
function analyzeSourceDiff(files) {
  ...
  for (const file of sourceFiles) {
    ...
    signals.totalAddedCommentLines +=
      meaningfulLines.filter(isCommentLine).length;
...
function summarizeFunctionChanges(functionNames) {
  ...
  if (capabilities.length > 0) {
    ...
    return {
      ...
      reason:
        "it produces more specific commit messages from actual code changes",
    };
  }
...
function buildMultiFileSourceContext(files, sourceSignals) {
  if (!sourceSignals.fileSignals || sourceSignals.fileSignals.length < 2)
    return null;

  const snippets = [];
  for (const fs of sourceSignals.fileSignals) {
    if (
      fs.file.includes("scripts/generate-commit-message") &&
      fs.functions.length > 0
    ) {
    ...
    if (fs.functions.length === 1) {
      snippets.push(
        `add ${formatFunctionLabel(fs.functions[0])} helper in ${fileLabel(fs.file)}`,
      );
      continue;
    }
    ...
    if (fs.functions.length > 1) {
      snippets.push(
        `add ${fs.functions.length} helpers in ${fileLabel(fs.file)}`,
      );
      continue;
    }
...
function normalizeContext(context, type, files) {
  const whatChanged = toSingleLine(
    context.whatChanged || context.summary || inferSummary(type, files),
  );
  const whyChanged = toSingleLine(
    context.whyChanged || context.reason || inferReason(type),
  );
  const conciseSummary = toConciseSummary(
    context.conciseSummary || context.summary || whatChanged,
  );
...
function normalizeAiContext(aiData, fallbackType, fallbackScope) {
  ...
  const type = toSingleLine(
    aiData.type || fallbackType || "chore",
  ).toLowerCase();
  const scope = toSingleLine(aiData.scope || fallbackScope || "");
  const conciseSummary = toConciseSummary(
    aiData.conciseSummary || aiData.summary || "update code",
  );
  const whyChanged = toSingleLine(
    aiData.whyChanged || aiData.why || "it improves the implementation",
  );
...
async function requestOpenAiCommitContext({
  files,
  diff,
  stat,
  fallbackType,
...
function inferType(files) {
  ...
  const hasConfigOnly = files.every(
    (file) =>
      file === "package.json" ||
...
function inferReason(type) {
  ...
  if (type === "chore")
    return "it keeps project tooling and configuration aligned";
  return "it supports the new behavior";
}

function inferDetailedContext(type, files) {
  const packageDiff = files.includes("package.json")
    ? getFileDiff("package.json")
    : "";
  ...
  const usesCompressionMiddleware =
    /\+\s*this\.app\.use\(compression\(\)\)/.test(serverDiff);
  const importsCompression = /\+\s*import .*compression/.test(serverDiff);

  if (
    addedCompressionDep &&
    (usesCompressionMiddleware || importsCompression)
  ) {
    return {
      type: "feat",
      scope: "presentation",
      ...
      whatChanged:
        "integrated compression middleware in the Express server pipeline",
      whyChanged:
        "it reduces response payload size and improves API performance",
    };
  }
  ...
  if (addedDeps.length > 1 && addedCompressionDep && addedCompressionTypes) {
    return {
      ...
      whyChanged:
        "it enables compressed HTTP responses with TypeScript support",
    };
  }
  ...
  if (sourceSignals.commentOnly) {
    return {
      ...
      whyChanged:
        "it clarifies middleware behavior and server responsibilities for maintainers",
    };
  }
...
async function buildMessage() {
  ...
  try {
    ...
    console.error(
      "Could not read staged files. Are you inside a git repository?",
    );
    process.exit(1);
  }
  ...
  if (!files.length) {
    console.error(
      "No staged changes found. Stage files first, then run npm run commit:msg",
    );
    process.exit(1);
  }
